...
class PostgreSQLAdapter extends _socket.Adapter {
  constructor(nsp, uri, opts = {}) {
    super(nsp); // this.nsp = nsp;

    this.pg = new _pgPubsub.default(uri);
    this.uid = (0, _uuid.v4)();
  ...
  onmessage(msg) {
    console.log(`onmessage()`, msg); // ignore its own messages
  ...
  async delAll(id) {
    ...
    await Promise.all(promises); // await super.delAll(id);
  }
